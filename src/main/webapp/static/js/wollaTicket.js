function WollaTicketShow() {
    ...
    $.ajax({
        type: "post",
        url: url + "/wollaTicket/getWollaTicketList.action",
        ...
        success: function (data) {
            ...
            BusinessName.insertAdjacentHTML("beforeEnd", data.data.BusinessName);
...
//弹出删除框
function ShowDeleteChar(WollaTicketID) {
    var mymessage = confirm("确定要删除该？删除后不可恢复！");
    if (mymessage) {
        ConfirmDel(WollaTicketID);
...
//确认删除
function ConfirmDel(WollaTicketID) {
    var PutData = "BusinessLoginID="+BusinessLoginID;
    $.ajax({
        type: "POST",
        url: url+"/wollaTicket/deleteWollatTicket.action",
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        data: PutData,
        success: function (data) {
            if (data.code == 0) {
...
function GetBusinessNameList() {
    $.ajax({
        type: "post",
        url: url + "/bus/getBusinessNameList.action",
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        data: PutData,
        dataType: "json",
        success: function (data) {
            ...
            if (data.code == 0) {
                if (data.data.BusinessInfo.length == 0) {
                    ...
                    jQuery.each(data.data.BusinessInfo, function (i, list) {
                        str = str + '<option value="' + list.ID + ":" + list.BusinessName + '">' + list.BusinessName + '</option>';
                    });
